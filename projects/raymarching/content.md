...
I decided against using Shadertoy so I could gain a better understanding of the underlying technology and so I could embed my work in this writeup.
...
WebGL is a widely used API for interfacing with the GPU and the one I used for my experiments. It is a brother of OpenGL, but focused on browser based uses. I'll give a quick summary of the parts of WebGL that are important to us.

The programs we use to interface with the GPU are called shaders, and come in two primary types. The first is a vertex shader. A vertex shader takes is given the vertices in a scene and does some computation based on them, most often converting them from 3D world space to 2D screen space. After the vertex shader has completed, the triangles are passed to a rasterizer. The rasterizer determines which pixels are inside the triangles defined by the vertices and generates a "fragment" for each one. These fragments are passed into the second type of shader: a fragment shader. The fragment shader decides the color of the pixel, and is where the ray marching algorithm is run.
...
Simply returns the world coordinates of the vertex as clip space coordinates. By generating a rectangle of size (2, 2), centering it on the xy plane, and applying the vertex shader, I create a rectangle that permanently covers the entire screen. Here's the relevant THREE code:
...
Before we can ray march we need to determine what ray we actually want to march along. Imagine your screen is a window, and you are looking out into the scene. The ray from your eye
...
Once we have the ray, executing the actual ray marching is relatively simple:
...
We can reintegrate the information about the camera's orientation into the 

 THREE provides the camera's view matrix through a variable aptly named viewMatrix, and we can transform out ray using a matrix multiplication.
